...
class AuthService {
    ...
    private authenticatedCallback(authResult: any): void {
        localforage.setItem(TOKEN_NAME, authResult)
            .then(data => {
                localStorage.setItem(TOKEN_NAME, authResult.idToken);
                infoConsoleGroup('auth0-lock: authenticated', 'Autenticacion exitosa', data);
    ...
    logout(): void {
        localforage.removeItem(TOKEN_NAME)
            .then(() => {
                ...
                infoConsoleGroup('auth0-lock: logout', 'Se removió informacion del token de autenticación', TOKEN_NAME)
            })
